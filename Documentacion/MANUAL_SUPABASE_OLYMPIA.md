...
Las nuevas skills se inyectan en los workflows para garantizar una base de datos de "Grado Producción".

### Fase 1: `/quality_validation` (Optimización Postgres)
- **Tier 1 (Estático)**: La skill **`supabase-postgres-best-practices`** audita los esquemas de Prisma y SQL. Se verifican índices faltantes, tipos de datos ineficientes y se aplican las 8 categorías de optimización de Supabase.
...
- Se utiliza la skill **`supabase`** para realizar auditorías de seguridad en la configuración de Auth y Storage.
- Verificación de JWT, expiración de sesiones y permisos de buckets para evitar fugas de información.

### Fase 3: `/qa-review` (Revisión de Arquitectura)
...
## 3. Operaciones Especializadas con Skills
...
2. El agente activará la skill `supabase-postgres-best-practices` y generará un reporte de hallazgos categorizado por impacto (Crítico, Alto, Medio, Bajo).

### Gestión de Auth y Migraciones
1. Para configurar nuevos flujos de Auth (ej. OTP, Social Login): El agente consultará la skill `supabase` para obtener la implementación exacta compatible con `@supabase/ssr`.
