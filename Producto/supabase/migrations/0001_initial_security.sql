...
--------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION get_auth_restaurant_id() RETURNS text AS $$
...
CREATE POLICY self_restaurant_access ON "restaurants" FOR ALL USING (
    id::text = get_auth_restaurant_id ()
);
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('CREATE POLICY tenant_access_policy ON public.%I 
                    FOR ALL USING (restaurant_id::text = get_auth_restaurant_id())
                    WITH CHECK (restaurant_id::text = get_auth_restaurant_id())', t);
  END LOOP;
END $$;

DROP POLICY IF EXISTS users_tenant_access ON "users";
CREATE POLICY users_tenant_access ON "users"
  FOR ALL USING (restaurant_id::text = get_auth_restaurant_id() OR id::text = auth.uid()::text);
...
BEGIN
    ...
    CASE OLD.status
        WHEN 'PENDING' THEN
            IF NEW.status NOT IN ('VALIDATED', 'REJECTED') THEN RAISE EXCEPTION 'Transición inválida: PENDING -> %', NEW.status; END IF;
        WHEN 'VALIDATED' THEN
            IF NEW.status NOT IN ('PREPARING', 'REJECTED') THEN RAISE EXCEPTION 'Transición inválida: VALIDATED -> %', NEW.status; END IF;
        WHEN 'PREPARING' THEN
...
--------------------------------------------------------------------------------
-- IMPORTANTE: Usamos "createdAt" entre comillas dobles porque Prisma no lo mapeó a snake_case
