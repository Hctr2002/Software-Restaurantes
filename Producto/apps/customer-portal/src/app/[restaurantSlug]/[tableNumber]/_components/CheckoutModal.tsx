  ...
  return (
    <div className="fixed inset-0 z-[60] animate-in fade-in duration-300">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-card/95 backdrop-blur-xl rounded-t-[2.5rem] p-8 max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom duration-500 border-t border-foreground/10 shadow-[0_-20px_50px_rgba(0,0,0,0.3)]">
            ...
            <div key={item.id} className="flex justify-between items-center py-3 border-b border-foreground/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground/40">${item.price.toLocaleString()}</p>
                </div>
              </div>
              <span className="text-foreground font-bold">${(item.price * item.quantity).toLocaleString()}</span>
            </div>
          ))}
        </div>
        ...
        {table.data ? (
          <div className="bg-primary/5 border border-primary/20 rounded-2xl p-6 mb-8 flex items-center justify-between animate-in fade-in zoom-in">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="text-foreground font-bold text-lg leading-tight">Ubicación Confirmada</h4>
                <p className="text-primary font-medium text-sm">Tu pedido llegará aquí.</p>
              </div>
            </div>
            <div className="text-right">
              <span className="block text-[10px] uppercase font-black tracking-widest text-foreground/40 mb-1">Mesa</span>
          ...
          <div className="bg-destructive/10 border border-destructive/30 rounded-2xl p-6 mb-8 flex items-center gap-4 animate-in fade-in zoom-in">
            ...
            <div>
              <h4 className="text-foreground font-bold text-sm leading-tight">Mesa no detectada</h4>
              <p className="text-destructive font-medium text-xs mt-0.5">Escanea el código QR de tu mesa para continuar.</p>
            </div>
          </div>
        )}

        <div className="bg-foreground/[0.03] rounded-2xl p-6 mb-8 border border-foreground/5">
          <div className="flex justify-between items-center mb-2"><span className="text-foreground/60">Subtotal</span><span className="text-foreground font-medium">${cartTotal.toLocaleString()}</span></div>
          {table.data && <div className="flex justify-between items-center mb-2"><span className="text-foreground/60">Mesa</span><span className="text-primary font-semibold">#{table.data.number}</span></div>}
          <div className="flex justify-between items-center pt-4 border-t border-foreground/10"><span className="text-foreground font-bold text-lg">Total</span><span className="text-primary font-black text-2xl">${cartTotal.toLocaleString()}</span></div>
        </div>
        ...
        <button onClick={onClose} className="w-full mt-4 text-foreground/40 hover:text-foreground/60 font-medium py-2 transition-colors">Continuar comprando</button>
      </div>
    </div>
  );
}
