...
export function BillAlertIsland({ groups, onSelect }: Props) {
  const alertGroups = groups.filter((g) => g.billRequested);
      ...
      <AnimatePresence>
        {alertGroups.map((g) => (
          ...
          >
            ...
            <button
              onClick={() => onSelect(g)}
              className="h-8 px-3 rounded-lg bg-yellow-500 text-black text-[9px] font-black hover:bg-yellow-400 transition-all"
            >
              COBRAR
            </button>
          </motion.div>
