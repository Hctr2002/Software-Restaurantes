      ...
      actions={
        <div className="flex items-center gap-4">
          <div className="relative flex items-center h-12">
            <AnimatePresence>
              {isSearchExpanded && (
                <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: isSearchExpanded ? (typeof window !== "undefined" && window.innerWidth < 640 ? 160 : 240) : 0, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="overflow-hidden mr-2">
            ...
            <button onClick={onSearchToggle} className={`p-2.5 rounded-xl transition-all ${isSearchExpanded ? "bg-primary text-primary-foreground" : "bg-foreground/5 text-muted-foreground hover:text-foreground"}`}>
              <Search className="w-5 h-5" />
            </button>
          </div>

          <div className="flex items-center bg-foreground/5 rounded-2xl p-1 border border-foreground/10">
            <button onClick={onRefresh} className="p-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-foreground/5 transition-all active:scale-90" title="Actualizar">
