...
export function useWebPush(restaurantId: string | undefined, readyOrders: Order[], pendingOrders: Order[] = [], billRequestedCount: number = 0, helpRequestedCount: number = 0) {
  const prevCount = useRef(0);
  const { audioBlocked, enableAudio } = useNotificationSound({ count: readyOrders.length + pendingOrders.length + billRequestedCount + helpRequestedCount });
