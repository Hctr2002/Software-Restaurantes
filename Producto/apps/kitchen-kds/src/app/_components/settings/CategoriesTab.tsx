...
export function CategoriesTab({ draft, categories, loadingMenu, onChange }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {draft.categoryTimes.length === 0 && <p className="text-white/20 text-sm text-center py-6">Sin categorías configuradas</p>}
      ...
      <div className="flex space-x-3 items-end pt-2">
        {loadingMenu ? (
          ...
          <select className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm" value={newCatName} onChange={(e) => setNewCatName(e.target.value)}>
        ...
        <div className="flex items-center space-x-2 bg-white/5 border border-white/10 rounded-xl px-4 py-3">
          <input type="number" min={1} max={120} value={newCatMinutes} onChange={(e) => setNewCatMinutes(Number(e.target.value))} className="w-12 bg-transparent text-white text-sm text-center outline-none" />
